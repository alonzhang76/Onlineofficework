/**
 * pdf-share.js — 业务单据 PDF 生成与分享（合同/报价单通用）
 * 离屏 Canvas 页面 → JPEG → PDF 1.4 → 写临时文件 → wx.shareFileMessage 转发微信好友
 * 渲染器为异步（需等待图片加载），因此 pages 支持 Promise。
 */
 ...
 */
function sharePages(pagesOrPromise, fileName, done, loadingTitle) {
  wx.showLoading({ title: loadingTitle || '正在生成PDF…', mask: true });
