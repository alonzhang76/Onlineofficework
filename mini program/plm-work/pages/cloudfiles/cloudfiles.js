 ...
 *   - 三应用共享同一云端根 orderschedule/（与网页版 cfb.js CB_ROOT 一致，一处上传三处可见）
 */
...
Page({
  ...
  refresh(done) {
    ...
    cbFiles.list(this.data.cur, this.data.app).then(res => {
      ...
      // 根目录固定显示与网页版一致的六个分区文件夹：云端分区里还没有文件时，
      // 云函数列不出该目录（网页版用 .emptyFolderPlaceholder 占位），这里补齐；
      // 云函数返回的同名目录去重，自定义目录排在分区之后。
      if (!this.data.cur) {
        const cloudByName = {};
        res.folders.forEach(f => { cloudByName[f.name] = f; });
        const merged = ZONES.map(z => cloudByName[z.folder] || { name: z.folder, path: z.folder + '/' });
        res.folders.forEach(f => {
          if (!ZONES.some(z => z.folder === f.name)) merged.push(f);
        });
        rawFolders = merged;
      }
      this.setData({
        ...
        files: res.files.filter(f => f.name !== PLACEHOLDER).map(f => Object.assign({}, f, {
          sizeText: this.fmtSize(f.size),
          dateText: this.dateText(f.lastModified),
  ...
  /** 云函数返回 ISO 时间字符串（如 2026-09-13T12:26:13.000Z），格式化为本地时间 */
  dateText(iso) {
    if (!iso) return '';
