 ...
 *
 * 修复：原实现把整个命名空间的 key 合并成一条 id=in.(...) 请求，
 * wage 命名空间响应体可达 2.8MB，真机 wx.request 会截断/失败，
 * 导致"拉取成功"但数据是空/旧的。现改为逐键请求，每个响应远小于 1MB，
 * 并兼容微信单键 1MB 存储上限（配合 safeGet/safeSet 分片）。
 *
...
function pullAll(ns) {
  ...
  // 逐键拉取：串行 + 小并发（每批 3 个），既避免超大响应，又不至于太慢
  const batchSize = 3;
  const result = [];
  let idx = 0;
  return new Promise((resolve, reject) => {
    function nextBatch() {
      ...
      Promise.all(batch.map(k => {
        ...
        return req('GET', '/v1/rdb/rest/' + CONFIG.table,
          ...
          .catch(err => {
            console.warn('[cloudbase] 拉取 ' + k + ' 失败:', err && err.errMsg || err);
            return null;
          });
      })).then(items => {
 ...
 * wx.setStorageSync 单个 key 最大约 1MB。wage_records 等大数组序列化后
 * 可达 2~3MB，直接写入会被静默拒绝，导致每次冷启动都回退到旧数据。
 * safeSet/safeGet 自动把超过阈值的值拆成多个分片 key 存储。
 */
const SHARD_THRESHOLD = 900 * 1024;   // 单 key 超过 900KB 即分片（留余量）
const SHARD_CHUNK = 800 * 1024;       // 每个分片约 800KB
...
function safeSet(key, value) {
  try {
    ...
    try {
      ...
      if (oldMeta && oldMeta.count) {
        for (let i = 0; i < oldMeta.count; i++) {
          try { wx.removeStorageSync(_shardKey(key, i)); } catch (e) {}
        }
      }
    ...
    if (json.length <= SHARD_THRESHOLD) {
      wx.setStorageSync(key, value);
      return true;
    }
    // 分片：按字符数切（JSON 字符串），用 string slice 即可
    const chunks = [];
    for (let pos = 0; pos < json.length; pos += SHARD_CHUNK) {
      chunks.push(json.slice(pos, pos + SHARD_CHUNK));
    }
    for (let i = 0; i < chunks.length; i++) {
      wx.setStorageSync(_shardKey(key, i), chunks[i]);
    }
    ...
    console.warn('[cloudbase] safeSet 失败 key=' + key, e && e.errMsg || e);
    return false;
  }
}
  ...
  try {
    ...
    if (meta && meta.count) {
      ...
      for (let i = 0; i < meta.count; i++) {
        const part = wx.getStorageSync(_shardKey(key, i));
        if (part === '' || part === undefined || part === null) return null;
        parts.push(part);
      }
