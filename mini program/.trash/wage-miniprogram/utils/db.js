/**
 * 数据层 —— 与 Web 版 wage app 完全同构的数据模型与业务逻辑
 * 存储：wx storage（键名与 Web 版 localStorage 一致）
 * 同步：每次保存异步推送 Supabase（见 utils/supabase.js）
 */
...
function save(key) {
  ...
  // 异步推送云端（未配置 Supabase 时静默跳过）
  supa.push(KEYS[key], data[key]).catch(err => console.warn('[supabase] push failed', KEYS[key], err));
}
...
function syncFromCloud(cb) {
  ...
  supa.pullAll().then(rows => {
    ...
    cb(true);
  }).catch(err => { console.warn('[supabase] pull failed', err); cb(false); });
}
