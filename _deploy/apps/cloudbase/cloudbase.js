 ...
 * 由腾讯云开发 CloudBase（https://tcb.cloud.tencent.com）提供数据库/存储/认证。
 *
 * 两种自动登录模式（init 时按页面预设选择）：
 *   1. 无登录表单的应用：在引入本模块前设置
 *        window.CLOUDBASE_SYNC = { email: 'xxx', password: 'xxx' }
 *      模块会用该共享账号静默登录（authenticated，可读写）；
 *      失败时回退匿名登录（anon，仅只读）。由 cloudbase-sync.js 自动设置。
 *   2. 门户登录页：设置 window.CLOUDBASE_DISABLE_ANON = true，
 *      不做任何自动登录，完全由用户输入邮箱密码登录。
 *   3. 默认（如 wage）：未登录时匿名登录保证共享数据可读。
 *
 * 工作原理：
...
/* ---------- 启动期认证引导 ----------
 * 三种模式（见文件头说明）：
 *   CLOUDBASE_DISABLE_ANON = true → 不做任何自动登录（门户登录页）
 *   CLOUDBASE_SYNC = {email,password} → 共享账号静默登录（无登录表单的应用）
 *   默认 → ensureLogin 匿名登录（wage 等共享只读场景）
 */
async function bootstrapAuth(app) {
  // 1) 门户登录页：完全不自动登录，避免匿名会话与用户邮箱登录竞争
  if (typeof window !== "undefined" && window.CLOUDBASE_DISABLE_ANON) {
    return;
  }

  // 2) 无登录表单应用：共享账号静默登录，拿到 authenticated 角色以读写数据
  var syncCfg = (typeof window !== "undefined" && window.CLOUDBASE_SYNC) || null;
  if (syncCfg && syncCfg.email && syncCfg.password) {
    try {
      ...
      var userS = resS && resS.data && resS.data.user;
      if (resS && resS.error || !userS) {
        console.warn("[cloudbase.js] 共享账号登录失败，回退匿名只读:", (resS && resS.error && resS.error.message) || "");
        await ensureLogin(app);
        return;
      }
      console.log("[cloudbase.js] ✅ 共享账号已登录（authenticated，可读写）:", userS.email || syncCfg.email);
      return;
    } catch (eSync) {
      console.warn("[cloudbase.js] 共享账号登录异常，回退匿名只读:", eSync && eSync.message ? eSync.message : eSync);
      await ensureLogin(app);
    }
    return;
  }
